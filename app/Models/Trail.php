...
class Trail extends Database {
    ...
    public function updateTrail($id, $name, $length, $difficulty, $longitude_A, $latitude_A, $longitude_B, $latitude_B, $img) {
        try {
            $stmt = $this->pdo->prepare("UPDATE trail SET name = :name, length = :length, difficulty = :difficulty, longitude_A = :longitude_A, latitude_A = :latitude_A, longitude_B = :longitude_B, latitude_B = :latitude_B, img = :img WHERE id = :id");
            ...
            if ($stmt->execute()) {
                ...
                // Si la requête échoue, affichez les informations d'erreur
